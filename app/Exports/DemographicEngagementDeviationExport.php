...
class DemographicEngagementDeviationExport implements FromView, WithHeadings, WithTitle, ShouldAutoSize, WithStyles
{
    private $group;
    ...
    public function view(): View
    {
        ...
        $data = array_column($data, 'deviation', 'group_id');
        ...
        foreach ($data as $groupId => $deviation) {
            $colors[$groupId] = $this->getColor($deviation);
        }
        ...
        return view('Exports.EngagementDeviation', compact('groupName', 'data', 'groupData', 'colors'));
    }
    ...
    {
        $query = str_replace('{{$var}}', $this->group, file_get_contents(app_path('Queries/demographic_engagement_deviation.sql')));
        return $query;
    }
    ...
    {
        $colName = DemographicsMap::getColumnName($this->group);
        $title = "Engagement - Desviación por {$colName}";
        return $title;
    }
