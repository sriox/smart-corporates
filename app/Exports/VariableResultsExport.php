...
class VariableResultsExport implements FromView, WithHeadings, WithTitle, ShouldAutoSize, WithStyles
{
    private $group;
    ...
    public function view(): View
    {
        ...
        foreach ($data as $item) {
            $results[$item->question_id][$item->group_id] = $item->result;
            $colors[$item->question_id][$item->group_id] = $this->getColor($item->result);
        }

        return view('Exports.VariableResults', compact('groupData', 'questions', 'results', 'colors'));
    }
    ...
    public function collection()
    {
        ...
        $collection = collect(array_map(function ($reg) {
            ...
            return [$reg->group, $reg->question_variable, $reg->result];
        }, $data));
        return $collection;
    }
    ...
    {
        $query = str_replace('{{$var}}', $this->group, file_get_contents(app_path('Queries/variable_results.sql')));
        return $query;
    }
    ...
    {
        $colName = DemographicsMap::getColumnName($this->group);
        $title = "Resultados por {$colName} - Indicador";
        return $title;
    }
